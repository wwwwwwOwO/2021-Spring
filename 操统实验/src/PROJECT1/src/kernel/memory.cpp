...
int MemoryManager::allocatePages(enum AddressPoolType type, const int count)
{
    ...
    for (int i = 0; i < count; ++i, vaddress += PAGE_SIZE)
    {
        ...
        process->user_resident_set_size++;
        ...
        //将页面换出内存
        if(type == AddressPoolType::USER && process->user_resident_set_size >= process->user_resident_set_maxsize){
            swapManager.swapOut(vaddress);
        }
