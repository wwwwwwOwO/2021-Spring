...
int ProgramManager::executeProcess(const char *filename, int priority, int resident_set_maxsize)
{
    bool status = interruptManager.getInterruptStatus();
    interruptManager.disableInterrupt();
    ...
    // 初始化进程的内存块描述符
    memoryManager.block_desc_init(process->user_block_descs);

    process->user_resident_set_maxsize = resident_set_maxsize;
    process->user_resident_set_size = 0;
...
int ProgramManager::fork()
{
    ...
    int pid = executeProcess("", 0, parent->user_resident_set_maxsize);
    if (pid == -1)
    {
        interruptManager.setInterruptStatus(status);
        return -1;
